...
export class HUD {
  constructor(root, state, bus, loop, { onHelp, onStats, onMute, onSave, board } = {}) {
    this.root = root;
    this.state = state;
    this.bus = bus;
    this.loop = loop;
    ...
    root.innerHTML = `
      ...
      <div class="hud-tools" role="toolbar" aria-label="Game controls">
        <button class="hud-toolbtn save-btn" title="Save now (autosave runs in the background)" aria-label="Save game">SAVE</button>
        <button class="hud-toolbtn mute-btn" title="Mute / unmute (M)" aria-label="Toggle sound">SND</button>
  ...
  _renderMuteIcon(muted) {
    ...
    this.el.mute.textContent = muted ? 'OFF' : 'SND';
